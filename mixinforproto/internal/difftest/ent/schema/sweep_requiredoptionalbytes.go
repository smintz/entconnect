...
// RequiredOptionalBytes is Plan 03-05 Task 3's PIPE-06 differential-sweep fixture: a
// bare MixinForProto[*mixinforprototestv1.RequiredOptionalBytes]() with no options, so
// sweep_test.go can drive real Create()/Update() calls against it
// generically via reflection (ent.Mutation.SetField), without any
// RequiredOptionalBytes-specific Go code in this package. See internal/difftest/
...
func (RequiredOptionalBytes) Mixin() []ent.Mixin {
	return []ent.Mixin{
		mixinforproto.MixinForProto[*mixinforprototestv1.RequiredOptionalBytes](),
	}
}
