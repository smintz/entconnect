...
**Project:** entconnect (contract-first ent + ConnectRPC codegen toolkit)
**Researched:** 2026-08-08
**Confidence:** MEDIUM (all core versions verified against pkg.go.dev/GitHub directly; a few ecosystem items are WebSearch-only and marked LOW)
...
go get github.com/cel-expr/cel-go/cel@v0.31.0    # transitively pulled by protovalidate/cel, but pin directly since you compile standalone programs
...
# dev/test tooling
go get -d github.com/sebdah/goldie/v2
brew install bufbuild/buf/buf   # or go install github.com/bufbuild/buf/cmd/buf@v1.72.0
go install ariga.io/atlas-cli@v1.3.0 # or brew install ariga/tap/atlas — CLI, not a go.mod dependency
```

`go.work` at the repo root (not committed to either module's go.mod) to develop `mixinforproto` and the parent module together locally without `replace` directives:
...
These are the assumptions the downstream_consumer flagged as needing real verification. All were checked against pkg.go.dev / GitHub, not training data.
...
**If targeting Connect-only (v0.1–v0.3 of the roadmap, per entconnect.md §7):**
...
**If mixinforproto is extracted to a standalone repo later (design doc mentions this as a release-time decision):**
...
**If Tier 3 message-level CEL (`WithMessageRules(OnCreate)`) is implemented:**
...
---
*Stack research for: contract-first ent + ConnectRPC Go codegen toolkit*
*Researched: 2026-08-08*
