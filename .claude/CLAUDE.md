...
| github.com/google/cel-go | v0.28.0 | CEL compile/eval engine underneath protovalidate | **Corrected 2026-08-14 (Phase 3, D-16):** this is the exact module and version `buf.build/go/protovalidate@v1.2.0`'s own `go.mod` requires and both `go.mod` files in this repo already resolve (previously indirect; promoted to a direct `mixinforproto` dependency in Phase 3). Import path is `github.com/google/cel-go/cel`. Needed as a direct dependency because `buf.build/go/protovalidate/cel.NewLibrary()` returns a `cel.Library` that you feed into your own `cel.NewEnv(...)`. The earlier entry this row carried, naming the `cel-expr` fork at a later version, was verified wrong — see Load-Bearing Findings and 03-RESEARCH.md. |
...
| cel-go as a stable, unchanging dependency | **CORRECTED (Phase 3, D-16) — use `github.com/google/cel-go`, not `cel-expr/cel-go`** | `buf.build/go/protovalidate@v1.2.0`'s own `go.mod` requires `github.com/google/cel-go v0.28.0` directly and imports `github.com/google/cel-go/cel` throughout — verified against the downloaded module source (03-RESEARCH.md). `github.com/cel-expr/cel-go` is **not** a dependency of the pinned protovalidate version, and its own `go.mod` still declares the old path, so importing it produces a `cel.Library` type mismatch at compile time. **Import `github.com/google/cel-go/cel`, matching whatever `buf.build/go/protovalidate`'s own `go.mod` pins — re-verify this pairing if either module is bumped.** |
...
| `github.com/cel-expr/cel-go` | Not a dependency of `buf.build/go/protovalidate@v1.2.0` — its own `go.mod` still declares the old `github.com/google/cel-go` path, so importing it produces a `cel.Library` type mismatch against protovalidate's own env construction. Verified wrong (Phase 3, D-16); do not add it. | `github.com/google/cel-go`, matching whatever `buf.build/go/protovalidate`'s own `go.mod` pins |
...
- Its dependency set is already minimal and portable as researched: `ent`, `google.golang.org/protobuf`, `buf.build/go/protovalidate`, `github.com/google/cel-go/cel` (D-16). No buf CLI, no Connect, no genproto annotations — confirmed nothing in the Tier 1/2/3 validation relay mechanism requires them.
...
| buf.build/go/protovalidate@v1.2.0 | github.com/google/cel-go@v0.28.0 | `protovalidate/cel.NewLibrary()` returns a `cel.Library` typed against whichever cel-go import path protovalidate itself pins — check protovalidate's own go.mod at the version you lock to, and match your direct cel-go import to the same one, or `cel.Library` type mismatches will surface as compile errors, not runtime ones. Corrected 2026-08-14 (Phase 3, D-16): the pairing is `github.com/google/cel-go`, not `cel-expr/cel-go`. |
...
- github.com/google/cel-go, buf.build/go/protovalidate@v1.2.0's go.mod (direct source read, Phase 3/03-RESEARCH.md) — confidence HIGH; supersedes this document's earlier `cel-expr/cel-go` claim
